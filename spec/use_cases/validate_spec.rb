...
RSpec.describe UseCases::Validate do
  ...
  context 'with invalid params' do
    ...
    it 'fails with :validation_failure' do
      ...
      subject.call(params, user) do |match|
        match.failure :validation_error do |(code, errors)|
          result = errors
        end
      ...
      expect(result).to eq({ value_checked_param: ["must be equal to some string"], required_string_param: ["must be a string"] })
    end    
  end
end